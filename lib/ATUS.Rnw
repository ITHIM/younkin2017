To increase the sample size, we pooled ATUS responses for the years
2011-2015.

\begin{figure}
<<atus1, echo = FALSE, eval = FALSE, warning = FALSE, message = FALSE, fig.height = 6>>=
...
ggplot(fiveYear, aes(x = SEX, y = log(METs), fill = AGECAT)) + geom_boxplot() + ylab("Leisure Activity (log-scale) (MET-hrs./week)") + theme_bw()

parameters.LA <- fiveYear %>% mutate(logLA = log(METs)) %>% group_by(SEX, AGECAT) %>% summarise( meanLogLA = mean(logLA), sdLogLA = sd(logLA))
