...
ggplot(fiveYear, aes(x = SEX, y = METs, fill = AGECAT)) + geom_boxplot() + theme_bw() + labs(x = "Sex", y = "Leisure Activity (natural log scale) (MET-hrs./week", fill = "Age")  + scale_y_continuous(trans = "log", breaks = breaks <- exp(seq(0,8, by = 1))) #, labels = comma(breaks, digits = 1))
