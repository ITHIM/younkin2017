...
parameters.TA.All <- OHAS.full %>% group_by(GEND, AGECLASS) %>% mutate(walk = ifelse(is.na(walk),0,walk), cycle = ifelse(is.na(cycle),0,cycle) ) %>% summarise( meanWalk = mean(walk), meanCycle = mean(cycle))
