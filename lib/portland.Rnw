...
increase in travel activity we target the Portland Metropolitan Area
and estimate the change in DALYs associated with an increase of the
proportion of active travelers.

In Figure \ref{portland8} we see that
...
@
\caption{The mean weekly travel time in the Portland Metropolitan Area
...
D.baseline <- getQuantilesMixture(p.AT = p.AT.portland, mean.AT = summaryStats$mean, sd.AT = summaryStats$sd, mean.NT = 5, sd.NT = 5)
D.scenario <- getQuantilesMixture(p.AT = 1, mean.AT = summaryStats$mean, sd.AT = summaryStats$sd, mean.NT = 5, sd.NT = 5)

D <- rbind(data.frame(D.baseline,vision = "baseline"),data.frame(D.scenario,vision = "scenario"))

ggplot(D, aes(x = quantiles, y = prob, colour = vision)) + geom_line(aes(x=quantiles,y=prob)) + coord_cartesian(xlim = c(0,150)) + theme_bw() + xlab("Physical Activity (MET-hrs./week)") + ylab("Cumulative Distribution")
