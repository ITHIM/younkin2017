...
p.AT.scenario.vec <- seq(p.AT.portland,1,length.out = 50)
...
ggplot(results, aes(x = p.AT.scenario, y = 242.8*af, color = mean.NT)) + geom_line(aes(x=p.AT.scenario,y=242.8*af)) + theme_bw() + xlab("Proportion of Active Travelers") + ylab("CVD Deaths Averted") #+ coord_cartesian(xlim = c(0,150))
