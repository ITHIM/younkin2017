...
inactive travel.  More than $x$ percent of people in the Portland
...
\caption{The mean weekly travel time (minutes) in the Portland Metropolitan Area
  stratified by age-class, sex and mode. (NHTS)}
\label{portland8}
\end{figure}
...
@
\caption{The proportion of inactive travelers in the Portland
  metropolitan area. (NHTS)}
\label{portland9}
\end{figure}
...
@
\label{mandm2223}
\caption{Mean travel activity (\METhrs{}) among active travelers in
  the Portland Metropolitan area. (NHTS)}
\end{figure}

\paragraph{Oregon Household Activity Survey}
<<ohas1, echo = FALSE, eval = TRUE, results = "hide", warning = FALSE, message = FALSE, fig.height = 3>>=
OHAS.df <- readRDS(file = "/Users/syounkin/Bullitt/data/OHAS/OHAS.df.rds")
...
We include the following counties in the following analysis: \Sexpr{countyList}.
...
trip.df <-  OHAS.df %>% filter( !(TPURP %in% c("20", "96")) & TRPDUR <= 60) %>% complete(ID,MODE) %>% filter(MODE %in% c("walk","cycle")) %>% select(ID, MODE, TPURP, TRPDUR)
...
#ggplot(D, aes(x = GEND, y = TA, fill = AGECLASS)) + geom_boxplot() + ylab("Travel Activity (MET-hrs./week)") + coord_cartesian(ylim = c(0, 150))

ggplot(subset(D, TA > 0), aes(x = GEND, y = TA, fill = AGECLASS)) + geom_boxplot() + ylab("Travel Activity (MET-hrs./week)") + coord_cartesian(ylim = c(0, 150))

## D <- full_join(
##   OHAS.df %>% mutate(AGECLASS = ITHIM:::convertToAgeClass(AGE)) %>% group_by(MODE,GEND,AGECLASS) %>% summarise(TOTDUR = sum(TRPDUR)) %>% filter(MODE %in% c("walk","cycle")) %>% arrange(MODE, GEND, AGECLASS) %>% spread(MODE,TOTDUR),
##   OHAS.df %>% mutate(AGECLASS = ITHIM:::convertToAgeClass(AGE)) %>% group_by(GEND,AGECLASS) %>% summarise(N=n_distinct(paste0(SAMPN,PERNO))), by = c("GEND","AGECLASS") ) %>% mutate(MUWT = walk/N, MUCT = cycle/N) %>% mutate( TA = alpha_w*MUWT + alpha_c*MUCT )

#ggplot(D, aes(x = TA, ..density.., colour = AGECLASS)) + geom_freqpoly(binwidth = 5) + facet_grid( GEND ~ .) + labs(x = "", colour = "Age") + coord_cartesian(xlim = c(0, 50))

#ggplot(D, aes(x = TA, colour = GEND)) + geom_freqpoly(binwidth = 5) + facet_grid( AGECLASS ~ .) + labs(x = "", colour = "Age") + coord_cartesian(xlim = c(0, 20))

#ggplot(D, aes(x = TA)) + geom_freqpoly(binwidth = 5) + coord_cartesian(xlim = c(0, 100))

#
@
\label{allCounties}
\caption{Travel Activity ($T = \alpha_w t_w + \alpha_c t_c$) for Portland
  Metro Area Active Travelers ($T > 0$) (OHAS)}
\end{figure}

\paragraph{Per person mean walk and bike times by county.}
<<totaldur, eval = TRUE, echo = FALSE, results = "hide", warning = TRUE, error = TRUE, message = TRUE>>=
...
\paragraph{AgeClass by Sex by Five Counties}
<<pertable, eval = TRUE, echo = FALSE, results = "hide", warning = TRUE, error = TRUE, message = TRUE>>=
AGECLASS <- ITHIM:::convertToAgeClass(OHAS.df$AGE)
@

\begin{figure}
<<sampSize, eval = TRUE, echo = FALSE, results = "hide", warning = TRUE, error = TRUE, message = TRUE, fig.height = 9>>=
...
@
\label{sampSize}
\caption{Sample size by age, sex and county (OHAS)}
\end{figure}

\paragraph{Household Table (HH)}
COUNTYNAME, INCOME \& RACE
<<hhtable, eval = TRUE, echo = FALSE, results = "hide", warning = TRUE, error = TRUE, message = TRUE>>=
...
\begin{figure}
<<mandm235, echo = FALSE, eval = FALSE, results = "hide", warning = FALSE, message = FALSE, fig.height = 3>>=
deaths.CVD <- 242.8
alpha.CVD <- log(0.93831942)
mean.NT.vec <- c(1,10,100); cv <- 1;
p.AT.scenario.vec <- seq(p.AT.portland,1,length.out = 5)
...
results <- within(results, mean.NT <- factor(mean.NT, levels = c("1","10","100"), labels = c("low","medium","high")))

ggplot(results, aes(x = p.AT.scenario, y = deaths.CVD*af, color = mean.NT)) + geom_line(aes(x=p.AT.scenario,y=deaths.CVD*af)) + theme_bw() + xlab("Proportion with At Least One Trip\nPortland, OR, Male, 45-59") + ylab("Cardiovascular Disease\nDeaths Averted") + scale_colour_discrete(name="Physical Activity")# + coord_cartesian(xlim = c(p.AT.portland,1), ylim = c(0,20))
...
\begin{figure}
<<mandm2356, echo = FALSE, eval = FALSE, results = "hide", warning = FALSE, message = FALSE, fig.height = 3>>=
...
\begin{figure}
<<pATbyMSA, echo = FALSE, eval = TRUE, results = "hide", warning = FALSE, message = FALSE, fig.height = 9, dev = c("pdf","png")>>=
...
#%>% summarise(pAT = sum(AT, na.rm = TRUE)/sum(!is.na(AT)),pAT.se = sqrt(pAT*(1-pAT)/sum(!is.na(AT)))) %>% arrange(pAT)
...
ggplot(D.pAT, aes(x = factor(msaName, levels = D.pAT$msaName), y = prop, fill = AT.level)) + geom_bar(stat = "identity") + theme_bw() + ylab("Proportion") + xlab("Metropolitan Statistical Area (MSA)") + coord_flip() #+ scale_y_continuous(limits=c(0,1), breaks=0:10*0.1)
