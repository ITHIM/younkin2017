...
To demonstrate the method for estimating health outcomes given two
travel scenarios we present here a brief case study of the
Portland Oregon, U.S.A., Metropolitan Area.

In Figure \ref{inactiveMSA} that although Portland has a great deal of
travel activity compared to other U.S. metropolitan areas, there is
still roughly 50\% of the Portland population that takes less than five
trips, recreational or travel, a week.

To perform the comparative risk assessment given here we need the
following parameters for both the baseline and scenario for each of
the $8 \times 2 \times 6 = 96$ age-sex-disease classes; proportion of
active travelers, mean log travel activity, standard deviation of log
travel activity, mean leisure activity, standard deviation of leisure
activity and dose-response shape parameter.  We use three data sources
to estimate these parameters, 2009 U.S. National Household
Transportation Survey (NHTS), 200?-200? American Time Use Survey
(ATUS) and 200? Oregon Household Activity Survey (OHAS).  Estimates
for the dose-response shape parameter, $\alpha$, are taken from the
original ITHIM implementation.

\subsection{Data Description}
\paragraph{National Household Transportation Survey}
\paragraph{American Time Use Survey}
\paragraph{Oregon Household Activity Survey (Eric or Brendan?)}
We include the following counties in the analysis: \Sexpr{countyList}.
...
travelers are active once every \Sexpr{nAT} days,
$\fAT = \Sexpr{round(fAT,2)}$, and scale accordingly,
$\hatpAT = \frac{1}{\fAT}p_{\{T>0\}}$, where $p_{\{T>0\}}$ is the
...
Figure \ref{TAFig} for box plots of travel activity stratified by age and
sex.  Note that the natural logarithm of travel activity is
displayed.

\paragraph{Estimates}
We excluded trips for recreation and loop trips\ldots
