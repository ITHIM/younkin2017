...
non-random, but among the active travelers we fit a log-normal
...
D <- full_join(trip.df, subject.df, by = "ID", suffix = c(".trip",".subject")) %>% group_by(ID, MODE, GEND, AGECLASS) %>% summarise(TOTDUR = sum(TRPDUR)) %>% ungroup() %>% spread(MODE,TOTDUR) %>% mutate(TA = 7/60*fAT*(alpha_w*ifelse(is.na(walk),0,walk) + alpha_c*ifelse(is.na(cycle),0,cycle)))
...
pAT %>% ggplot(aes(x = GEND, y = pAT, fill = AGECLASS)) + geom_bar(stat = "identity", position = "dodge") + theme_bw()
@
\caption{Proportion of inactive travelers ($T = 0$) in Portland Metro area}
\label{inactivePortland}
\end{figure}

\paragraph{Leisure Activity}\label{ATUS}
\Sexpr{knit_child('./ATUS.Rnw')}

\paragraph{Disease Burden}\label{burden}
