...
ggplot(subset(D, TA > 0), aes(x = GEND, y = log(TA), fill = AGECLASS)) + geom_boxplot() + ylab("Travel Activity (MET-hrs./week) (log-scale)") + theme_bw() # + coord_cartesian(ylim = c(0, 150))
...
Proportion of inactive travelers ($T = 0$).

\begin{figure}
<<inactivePortland, eval = TRUE, echo = FALSE, results = "hide", warning = TRUE, error = TRUE, message = TRUE, fig.height = 3>>=
D %>% group_by(GEND, AGECLASS) %>% summarise( pAT = sum(TA > 0, na.rm = TRUE)/length(TA)) %>% ggplot(aes(x = GEND, y = pAT, fill = AGECLASS)) + geom_bar(stat = "identity", position = "dodge") + theme_bw()
# + ylab("Proportion") + xlab("Metropolitan Statistical Area (MSA)") + coord_flip() #+ scale_y_continuous(limits=c(0,1), breaks=0:10*0.1)
@
\caption{Proportion of active travelers in Portland Metro area}
\label{inactivePortland}
\end{figure}

\paragraph{Leisure Activity}
