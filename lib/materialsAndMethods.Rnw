...
We consider physical activity that is vigorous enough and sustained
long enough to provide physoliogical benefit.  To that end we exclude
maintenance activites such as brushing teeth or taking a shower.  We
this in mind we decompose physical activity into three components;
occupational, leisure and travel.  The relative risk functions, used
here and in previous versions of ITHIM, exclude the occupational
component of physical activity, e.g., construction, waiting tables.
Thus, our exposure variable is the sum of travel and leisure activity.
These components are treated as two independent random variables.  An
empirical distribution for physical activity is then computed for the
comparative risk assessment.  Throughout this paper the term
``physical activity" implies non-occupational physical activity.

\subsubsection{Travel-Related Physical Activity}
...
and $T_c$ (hours), by $\alpha_w^\prime = 7\alpha_w$ and
$\alpha_c^\prime = 7\alpha_c$, respectively. The average workload of
walking and cycling in METs, $\alpha_w = 3$ and $\alpha_c = 4.5$ must
be scaled up to represent a full week of travel.  Note that we assume
that sampled day is a typical day.  (This assumption breaks down
because we know the sampled day is a work day.)

\begin{equation}
T = \alpha_w^\prime T_w + \alpha_c^\prime T_c
\end{equation}

\subsubsection{Inactive Travelers}
...
a point-mass at zero with probabilty ($\pNAT = 1 - \pAT$) and a
\logNormal{} distribution.  Estimating the proportion of inactive
travelers with one-day survey data is challenging so we instead used
questions L3 and L4 of Table 1 of the 2009 National Household
Transportation Survey which asked how many walking or cycling trips
were made in the previous week.  This will provide a more unbiased
estimate than with one-day data, but it is not clear if this value
includes recreational trips, and we also assume the estimate is
subject to inflation due to self-reporting bias.  Figure
\ref{inactiveMSA} shows the proportion of people who take zero walking
or biking trips to range frome approximately 40\% in Oklahoma City to
around 20\% in the Portland metropolitan area.

% The 2009 National
% Household Transportation Survey contained two questions related to the
% weekly frequency of walking and biking.  to estimate the proportion of
% active travelers across metropolitan regions, see Figure
% \ref{inactiveMSA}.


% Much of the U.S. metropolitan population does not walk or cycle at
% all, for travel or otherwise.
...
% \subsubsection{Leisure Activity}
...
% Survey (ATUS) data \cite{ATUS}.  For more discussion see Section \ref{LA}.


\subsection{Health Outcomes}

We use relative risk functions for each of the diseases included in
the model of the form $\R(x) = \mathrm{e}^{-\alpha \tilde{x}}$, where
$\tilde{x} = \sqrt{x}$ and $x$ is in \METhrs{} \cite{woodcock2009,
  monninkhof2007, harriss2009, hamer2009, jeon2007, hamer2007}.
