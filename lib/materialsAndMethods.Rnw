...
of adults in each age-sex cohort who do not walk or bike at all, we
used questions L3 and L4 of Table 1 of the 2009 National Household
Transportation Survey which asked how many walking or cycling trips
were made in the previous week, and computed the proportion of nonzero
responses.  We see in figure \ref{inactiveMSA} that much of the
U.S. metropolitan population does not walk or cycle at all.  Due to
the large numbers of inactive travelers in U.S. metropolitan areas we
model the distribution of travel activity as a mixture of a point-mass
at zero with probabilty ($\pi_0 = 1 - \pAT$) and a \logNormal{}
distribution for active travelers.  Note that the survey question does
not address if the trip is for recreation or travel.
...
Survey (ATUS) data \cite{ATUS}.  For more discussion see Section \ref{ATUS}.
