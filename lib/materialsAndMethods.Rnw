...
We see in Equation \ref{paf} $\af{}$ in terms of the distributions of
physical activity in the baseline and alternative scenarios,
$P_{ij}(x)$ and $Q_{ij}(x)$, respectively and the relative risks
$R_{ijk}(x)$. $R_{ijk}(x)$ represents the risk when the individual has
exposure $x$, physical activity (\mets) relative to the risk with zero
activity for age class $i$, sex $j$ and disease $k$.

A discretization of the intergals in Equation \ref{paf} results in an
approximation based on quantiles of physical activity.  The vectors
$\mathbf{x}$ and $\mathbf{x}^\prime$ represent quantiles for the
exposure distribution in baseline and alternative scenarios,
respectively.  In the original ITHIM implementation quintiles were
used for this approximation which resulted in instability in the
estimates of $\af{}$.
