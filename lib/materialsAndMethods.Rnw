...
implemntation in R we can now use any number of quantiles.  Originally
...
Sadly, many Americans do not receive any travel-related activity.
Using data from the National Househild Transportation Survey we
observe that the majortiy of Americans living in a metropolitan area
reported no active travel, see Figure \ref{InactiveNational}.  (Note
that this is an over-estimate of the true proportion of inactive
travelers.)  Due to the large proportion on inactive travelers we
...
@
\caption{The proportion of inactive travelers in the combined U.S. metropolitan area.}
\end{figure}
...
model using dose-response curves of the form
