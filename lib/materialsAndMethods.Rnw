...
at all.  Due to the large numbers of inactive travelers in
U.S. metropolitan areas we model the distribution of travel activity
as a mixture of a point-mass at zero with probabilty
($\pi_0 = 1 - \pAT$) and a \logNormal{} distribution for active
travelers.
