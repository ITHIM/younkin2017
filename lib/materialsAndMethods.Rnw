...
$\fAT$ be the expected weekly rate of active travel for an active
traveler (days with active travel per week).  We correct for the bias
by scaling $\pi_0$ by $\fAT$.  (Currently $\fAT=\frac{4}{7}$.  There is
currently nociperceptive correction being performed to this
estimator.))
