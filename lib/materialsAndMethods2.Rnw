...
\hat{\alpha_{\textrm{AT}}} &=& \hat{p_w}\alpha_w  + (1-\hat{p_w})\alpha_c
\end{eqnarray}

<<mandm21, echo = TRUE, eval = TRUE, results = "show", warning = FALSE, message = FALSE, fig.height = 3>>=
...
Assume $x$, travel activity, is distributed as a mixture of a
point-mass at zero and log-normal when x is not zero.  We refer to the
class of people with nonzero travel activity as active travelers.  The
proportion of active travelers in all US MSAs is \Sexpr{n.AT/n.ind}.

\paragraph{Active Travelers}
...
D <- NHTS.df %>% filter(id %in% AT.id.vec) %>% group_by(id) %>% summarise(totalDuration = log10(sum(duration)))
summaryStats <- D %>% summarise(mean = mean(totalDuration), sd=sd(totalDuration))
ggplot(D,aes(totalDuration)) + geom_histogram(aes(y=..density..), bins = 25, alpha = 0.4 ) + geom_line(aes(y = ..density.., colour = 'Empirical'), stat = 'density') + stat_function(fun = dnorm, args = list(mean = summaryStats$mean, sd = summaryStats$sd), aes(colour = 'Fitted Log-Normal')) + theme_bw()
...
ggplot(D, aes(x=quantiles, y = prob)) + geom_line(aes(x=quantiles,y=prob)) + ylim(c(0.80,1)) + theme_bw() + xlab("Travel Activity (MET-hrs./week)")
@
...
% STOPPED WRITING HERE. Thu Jun 29 11:52:10 2017
