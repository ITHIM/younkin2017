...
doseResponse <- within(doseResponse, GEND <- factor(GEND, levels = c("M","F")))
...
Create a scenario in which the proportion of active travleres
increases by 25\%.

<<parameters2, echo = FALSE, eval = TRUE, results = "hide", warning = FALSE, message = FALSE, fig.height = 3>>=
pSetList <- list()
...
\begin{figure}
<<parameters3, echo = FALSE, eval = TRUE, results = "hide", warning = FALSE, message = FALSE, fig.height = 3>>=
...
ggplot(healthOutcomes, aes(x = disease, y = diseaseDeltaBurden, fill = burdenType)) + geom_bar(stat = "identity", position = "dodge") + theme_bw()  + labs(x = "Disease", y = "Disease Burden Averted", fill = "Burden Type")
@
\caption{Health outcomes given a 25\% increase in proportion of active
travelers.}
\label{health}
\end{figure}
...
% <<functions, echo = FALSE, eval = FALSE, results = "hide", warning = FALSE, message = FALSE, fig.height = 3>>=
