...
pSet <- with(parameters,{
  new("ParameterSet", pAT = pAT[1], meanLogTA = meanLogTA[1], meanLogLA =
  meanLogLA[1], sdLogTA = sdLogTA[1], sdLogLA = sdLogLA[1], metWalk = 3,
  metCycle = 6, alpha = alpha[1], k = 0.5, quantiles = 1:99/100, label = "First Row")
})
