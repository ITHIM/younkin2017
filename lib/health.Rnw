...
Now that we have the baseline parameters estimated we can create an
alternative scenario with which to compare to baseline.  In this
example we set $\pAT=\Sexpr{round(pAT.scenario,2)}$ in each age-sex
class with $\pAT < \Sexpr{round(pAT.scenario,2)}$.  The mean travel
activity per age-sex class remains unchanged we simply increase the
proportion which are sampled from the \logNormal{} distribution,
instead of being set to zero in the model.
...
@
\caption{Health outcomes given a population with $\pAT > \Sexpr{round(pAT.scenario,2)}$.}
\label{healthFig}
\end{figure}

The averted disease burden in terms of DALY, YLL, YLD and deaths is
shown in Figure \ref{healthFig}.  In it we see that the greatest
impact occurs in cardiovascular disease (CVD) and dementia.
